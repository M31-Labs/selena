...
void main() {
  ...
  vec4 colorR = texture2D(_sceneColor, (v_uv - dispR));
  vec4 colorG = texture2D(_sceneColor, (v_uv - disp));
  vec4 colorB = texture2D(_sceneColor, (v_uv - dispB));
  vec4 lensed = vec4(colorR.r, colorG.g, colorB.b, 1.0);
  vec4 rawColor = texture2D(_sceneColor, v_uv);
  float distCenter = length(delta);
  float blendEdge = smoothstep(0.0, (softening * 4.0), distCenter);
  gl_FragColor = mix(rawColor, lensed, blendEdge);
}
