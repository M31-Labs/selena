...
void main() {
  ...
  vec4 e = texture(stateTex, (uv + vec2(texel, 0.0)));
  vec4 wv = texture(stateTex, (uv - vec2(texel, 0.0)));
  vec4 nn = texture(stateTex, (uv + vec2(0.0, texel)));
  vec4 ss = texture(stateTex, (uv - vec2(0.0, texel)));
  ...
  fragColor = vec4((mix(cool, warm, clamp((lit * 1.8), 0.0, 1.0)) * lit), 1.0);
}
